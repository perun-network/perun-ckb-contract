...
pub fn main() -> Result<(), Error> {
    ...
    for i in 0.. {
        match load_cell_type(i, Source::GroupInput) {
            ...
            Ok(None) => panic!("lockscript not found"),
    ...
    if counter == 1 {
        return honest_mode(); // normal mode is vc without dispute -> close?
    } else if counter == 2 {
        return dispute_mode(); // dispute mode -> merge two cells into one
    } else {
        return Err(Error::IndexOutOfBound); // not a legitimae
    }
}
...
// Note: This means, that each participant needs to use a secp256k1_blake160_sighash_all as input to interact with the channel.
// This should not be a substantial restriction, since a payment input will likely be used anyway (e.g. for funding or fees).
pub fn honest_mode() -> Result<(), Error> {
    for i in 0.. {
