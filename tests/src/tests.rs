...
fn test_vc_progress_no_update(context: Rc<Mutex<RefCell<Context>>>, env: &perun::harness::Env) -> Result<(), perun::Error> {
    let (alice, bob, ingrid) = ("alice", "bob", "ingrid");
    let alice_acc = random::account(alice);
    let bob_acc = random::account(bob);
    let ingrid_acc = random::account(ingrid);
...
fn test_vc_progress_update1(context: Rc<Mutex<RefCell<Context>>>, env: &perun::harness::Env) -> Result<(), perun::Error> {
    let (alice, bob, ingrid) = ("alice", "bob", "ingrid");
    let alice_acc = random::account(alice);
    let bob_acc = random::account(bob);
    let ingrid_acc = random::account(ingrid);
...
fn test_vc_merge(context: Rc<Mutex<RefCell<Context>>>, env: &perun::harness::Env) -> Result<(), perun::Error> {
    let (alice, bob, ingrid) = ("alice", "bob", "ingrid");
    let alice_acc = random::account(alice);
    let bob_acc = random::account(bob);
    let ingrid_acc = random::account(ingrid);
    ...
    create_vc_channel_test(context.clone(), env, &parts_ai, &parts_bi, |chan_ai, chan_bi| {
        ...
        chan_bi.with(ingrid).update(update_virtual_channel(&funding_agreement_ab, vc_ab_2.id().clone(), &idx_map.parent2));
        
        chan_ai.with(alice).vc_start(
            &mut vc_ab_1
        ).expect("vc_start by alice using C_AI");
        chan_bi.delay(delay);
        chan_bi.with(bob).vc_start(
            &mut vc_ab_2
        ).expect("vc_start by bob using C_BI");
        
        chan_bi.with(ingrid).vc_merge(
            &vc_ab_1,
            &vc_ab_2,
            0u8,
        ).expect("vc_merge");
...
fn test_vc_close2(context: Rc<Mutex<RefCell<Context>>>, env: &perun::harness::Env) -> Result<(), perun::Error> {
    let (alice, bob, ingrid) = ("alice", "bob", "ingrid");
    let alice_acc = random::account(alice);
    let bob_acc = random::account(bob);
    let ingrid_acc = random::account(ingrid);
    ...
    create_vc_channel_test(context.clone(), env, &parts_ai, &parts_bi, |chan_ai, chan_bi| {
        ...
        chan_ai.with(alice).vc_close1(&mut vc_ab, &idx_map_parent1).expect("vc_close1");
        
        let idx_map_parent2 = virtual_channel::IdxMapWithDirection{
            idx_map: idx_map.clone().invert_map(1 as usize),
            direction: IdxMapDirection::LedgerChannelToVirtualChannel
        };
        chan_bi.with(ingrid).vc_close2(&mut vc_ab, &idx_map_parent2).expect("vc_close2");
        chan_ai.assert();
        chan_bi.assert();
        Ok(())
    })          
